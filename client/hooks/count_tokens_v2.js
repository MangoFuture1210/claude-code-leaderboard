    ...
    while (retries < maxRetries && !success) {
      try {
        ...
        if (result.success) {
          ...
          results.push({ chunk: chunk.length, success: true });
        } else {
          retries++;
        ...
        retries++;
        if (retries >= maxRetries) {
          results.push({ chunk: chunk.length, success: false, error: error.message });
        } else {
          // 重试前等待
          await new Promise(r => setTimeout(r, 1000 * retries));
        }
...
async function collectAndSendUsage(config, logger) {
  ...
  try {
    ...
    if (result.success) {
      ...
      await clearBuffer();
      await logger.log('info', 'State updated successfully');
    } else {
      // 更新重试计数
      await updateBufferRetryCount();
      await logger.log('warn', 'Some entries failed to send, saved for retry');
    }
